...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/hms2'
db=SQLAlchemy(app)
...
class Emergency(db.Model):
    ...
    p_condition=db.Column(db.String(20))
    p_no=db.Column(db.String(20),primary_key=True)
...
def emergency():
    ...
    if request.method=="POST":
        ...
        query=Emergency(p_name=p_name,p_address=p_address,p_type=p_type,p_condition=p_condition,p_no=p_no)
        db.session.add(query)
        db.session.commit()
        flash("don't worry the ambulance on the way ","primary")

    return render_template('ambulane.html')
...
def doctors():
    ...
    if request.method=="POST":
        ...
        # query=db.engine.execute(f"INSERT INTO `doctors` (`email`,`doctorname`,`dept`) VALUES ('{email}','{doctorname}','{dept}')")
        query=Doctors(did=did,email=email,doctorname=doctorname,dept=dept)
        db.session.add(query)
        db.session.commit()
...
@app.route('/view_emergency', methods=['POST','GET'])
def view_emergency():
    query = Emergency.query.all()
    return render_template('emergency_view.html',query=query)

@app.route('/view_bills',methods=['POST','GET'])
def view_bills():
    query=Patients.query.all()
    return render_template('viewbill.html',query=query)

@app.route('/billing', methods=['POST','GET'])
def billing():
    if request.method == "POST":
        pid = request.form.get('pid')  # Corrected form field name to 'pid'
        bill = request.form.get('bill')
        
        # Retrieve the patient record from the database using the primary key 'pid'
        patient = Patients.query.filter_by(pid=pid).first()
        if patient:
            # Update the 'bill' column of the patient record
            patient.bill = bill
            
            # Commit the changes to the database
            db.session.commit()
            
            flash("Patient billed successfully", "success")
        else:
            flash("Patient with ID {} not found".format(pid), "error")
...
@app.route('/remove_doctor', methods=['POST','GET'])
def remove_doctor():
    if request.method == "POST":
        did = request.form.get('did')
...
@app.route("/delete_bill/<string:pid>", methods=['POST', 'GET'])
@login_required
def delete_bill(pid):
    query = Patients.query.filter_by(pid=pid).first()

    # Check if the query returned a patient with the given PID
    if query:
        # Set the 'bill' attribute to None
        query.bill = None
        ...
        flash("Patient with the given PID not found", "danger")

    return redirect('/view_bills')  # Redirect to the correct URL after deletion
